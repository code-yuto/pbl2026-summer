...
async def analyze_latest_sensor_reading(
    ...
    try:
        latest = await run_in_threadpool(repository.get_latest_reading)
    except Exception as error:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Unable to retrieve the latest live sensor reading",
        ) from error
    ...
    try:
        return await service.analyze_existing_and_store(
            request,
            int(latest["id"]),
        )
    except (
